---
title: "Lab7: Correlation and Linear Regression"
author: "Tatum Katz, Sam Sambado"
date: "2/14/2021"
output: html_document
---

### Biometry, EEMB 146, Week 7  
...
Correlation tells us (1) whether two variables are linearly associated with each other and (2) how they are associated (positively, negatively, or neither). For example, I may be interested in whether large bodied animals have larger brains: in other words, are body mass (*X*) and brain size (*Y*) correlated? To answer this question, I need to use something called *Pearson's correlation coefficient*, which is written as *r*. The *r* coefficient has the following properties:  
  
1. It has a value between -1 (perfect negative correlation) and 1 (perfect positive correlation). 0 indicates no correlation.  
2. It is defined as by the equation: $$r = \frac{ \Sigma(X - \bar{X})(Y-\bar{Y})}{\sqrt{\Sigma(X-\bar{X})^2}\sqrt{\Sigma(Y-\bar{Y})^2}}$$  
  + Notice that if we were interested in the correlation of X with itself (body mass with body mass) equation 1 would be 1: perfect positive correlation.  
3. *r* is only looking for a *linear* correlation between variables. It will not identify non-linear patterns.  
  
#### 1.2 Interference with *r*  
...
We can test the hypothesis that there is a significant correlation between two variables *X* and *Y* by asking whether $\rho$ (the true value of *r*) is equal to zero or not: $$H_0 : \rho = 0 \\ H_A: \rho \neq 0$$  
...
1. The observations are random samples (surprise, surprise..)  
...
Let's look at our kelp data set again and ask the following two questions:  
...
1. Load the dataset kelp_data.csv, *and name it kelp*
```{r}
#your code here
```  
...
#library(psych)
pairs.panels(plot.kelp, density = TRUE, cor=FALSE, lm=TRUE)  # makes a scatterplot matrix of your dataset. show density plots, do not show correlation values, use a linear model
...
pairs.panels(log.plot.kelp, density = TRUE, cor=FALSE, lm=TRUE)  # makes a scatterplot matrix of your dataset. show density plots (density = TRUE), do not show correlation values (cor = FALSE), use a linear model (lm = TRUE)
```  

You can see that the normality of the maximum and minimul kelp width is much better (we could test it with a Shapiro-Wilk). Moreover, there is no clear non-linear relationship that we can see between max_wid_log and min_wid_log. Therefore, let's proceed with a correlation test between these two variables:  
```{r}
# run a Pearson's correlation coefficient with a two sided hypothesis
cor.test(log.plot.kelp$max_wid_log, log.plot.kelp$min_wid_log, method="pearson", alternative="two.sided") 
```
First, look at *cor*. This is *r*. An *r* = 0.09 suggests an incredibly weak, positive correlation between these two variables. We can use our hypothesis test to look at this. The p-value for our hypothesis test is 0.3238, therefore, *we fail to reject H_0 that there is no significant correlation between log maximum kelp width and log minimum kelp width*.  
...
cor.test(plot.kelp$max_wid, plot.kelp$abund_amphipod, method="spearman", alternative="two.sided") 
```

First, look at $\rho$. This is the Spearman's correlation coefficient and, like *r*, it varies from -1 to 1. The positive value tells us that there is a positive association between our variables. Next, look at the p-value for our hypothesis test: it is so small! Therefore, *we reject the H_0 that there is no significant correlation between the ranks of amphipod abundance and the ranks maximum kelp width*. In other words, *there tend to be more amphipods under larger kelp wracks*.  
  
Bonus: want to see all correlations all ways?
```{r}
pairs.panels(kelp, density = TRUE, cor=TRUE, lm=TRUE, method="spearman")  # makes a scatterplot matrix of your dataset. shows density plots, runs spearman correlation and shows values, uses a linear model
```
...
1. The model is correct (we are not over-fitting or under-fitting the model). For each $X_i$, the corresponding $Y_i$ is a random sample (wow shocker).  
...
3. The variances of $Y_i$'s are the same for all values of $X_i$.  
  + Test by ensuring that the residual plot does not have a distinct pattern (see lab 7!)  
...
Load in the dataset deet.csv *and call it deet*: 
```{r}
#your code here
```  
  
Now, visualize the data with a scatterplot:  
```{r}
...
We now need to check our assumptions of normality and equal variance. To do that, let's make our linear regression model:  
```{r}
...
Just like with ANOVA, your model is now saved and ready to use! Let's test the model's assumptions:
```{r}
par(mfrow = c(2,2))
plot(deet.lm)
par(mfrow = c(1,1))
...
Just like with ANOVA, the second plot (Normal Q-Q) tests our assumption of normality. We can see that the QQplot of our residuals does not deviate much from the straight line so we can conclude that our assumption of normality is met. If we then look at the first plot (Residuals vs Fitted), we see that there is no distinct pattern in the residuals about the 0 line. Therefore, our equal variance assumption is met. We could do quantitative diagnostics if we wanted to, but these qualitative diagnostics looks pretty good. If our assumptions were not met we could try some transformations.  
...
2. Look at the column *Pr(>|t|) for your p-values.  
  ...
  + We see that the p-value for dose is p=1.74e-05. We can *reject our $H_0$ and say that the amount of DEET is a significant predictor of the number of mosquito bites (i.e. the slope is not 0)*. More specifically, *as we increase the dose of DEET, we see a significant decrease in mosquito bites*.  
...
Use R Markdown to answer these questions and upload your answers as HTML or PDF in the Lab 7 turn-in link on Gauchospace. 
